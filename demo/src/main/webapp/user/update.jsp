...
<!--      <li class="profile">
  ...
  <section class="home-section">
      ...
      <h1 style="text-align:center; padding-top:40px;">Cap nhat User</h1>
       <% User user = (User)request.getAttribute("user"); %>
      		<form method="post">
			  <div class="form-group">
			    <label>Ten <span style="color:red">*</span></label>
			    <input type="text" class="form-control" placeholder="Enter Username" value="<%= user.getName() %>" name ="name"  autofocus />
			    <span style="color:red"><%= errorName %></span>
			  </div>
			  <div class="form-group">
			    <label>Ma sinh vien <span style="color:red">*</span></label>
			    <input type="text" class="form-control" placeholder="Enter MaSV" name ="masv" value="<%= user.getMasv()%>">
			    <span style="color:red"><%= errorMasv %></span>
			  </div>
			  <div class="form-group">
				   <label>Khoa <span style="color:red">*</span></label>
				    <select name="khoa" class="form-control" >
				    	<option value="1" <%if(user.getKhoa() == 1){out.print("selected");} %>>CNTT</option>
				    	<option value="2" <%if(user.getKhoa() == 2){out.print("selected");} %>>ATTT</option>
				    	<option value="3" <%if(user.getKhoa() == 3){out.print("selected");} %>>DTVT</option>
				    	<option value="4" <%if(user.getKhoa() == 4){out.print("selected");} %>>DPT</option>
				    </select> 
				    <span style="color:red"><%= errorKhoa %></span>
			  </div>
			  <div class="form-group">
			    <label>Mat khau <span style="color:red">*</span></label>
			    <input type="password" class="form-control" placeholder="Enter Password" name ="password" value="<%= user.getPassword()%>">
			  ...
			  <div class="form-group">
			    ...
			    %>
			    <input type="date" class="form-control" name ="birthday" value="<%= dt1.format(user.getBirthday()) %>">
			    <span style="color:red"><%= errorBirthday %></span>
			  </div>
			  <div class="form-group">
			    <label>Que quan <span style="color:red">*</span></label>
			    <input type="text" class="form-control" placeholder="Enter Country" name ="country" value="<%= user.getCountry()%>">
			  	<span style="color:red"><%= errorCountry %></span>
			  </div>
			  <button type="submit" class="btn btn-primary">Cap nhat</button>
			</form>
			<div style="padding:20px 0px">
					<a  href="ListUser">Quay ve danh sach userr</a>
			</div>
      </div>
  </section>
  <script>
